...
export default function RadioButtons(props: RadioButtonsProps) {
  ...
  configProperty = configProperty.startsWith("@P")
    ? configProperty.substring(3)
    : configProperty;
  configProperty = configProperty.startsWith(".")
    ? configProperty.substring(1)
    : configProperty;
  ...
  const localeContext =
    metaData?.datasource?.tableType === "DataPage" ? "datapage" : "associated";
  const localeClass = localeContext === "datapage" ? "@baseclass" : className;
  const localeName =
    localeContext === "datapage" ? metaData?.datasource?.name : configProperty;
  ...
  const theOptions = Utils.getOptionList(
    theConfigProps,
    thePConn.getDataObject(""),
  );
    ...
    return (
      <FieldValueList
        ...
        value={thePConn.getLocalizedValue(
          value,
          localePath,
          thePConn.getLocaleRuleNameFromKeys(
            localeClass,
            localeContext,
            localeName,
          ),
