...
import {
  ...
  insertInstruction,
  deleteInstruction,
} from "@pega/react-sdk-components/lib/components/helpers/instructions-utils";

export const setVisibilityForList = (c11nEnv, visibility) => {
  const { selectionMode, selectionList, renderMode, referenceList } =
    c11nEnv.getComponentConfig();
  // usecase:multiselect, fieldgroup, editable table
  if (
    (selectionMode === PCore.getConstants().LIST_SELECTION_MODE.MULTI &&
      selectionList) ||
...
const preProcessColumns = (columns) => {
  return columns?.map((col) => {
    ...
    tempColObj.value =
      col.value && col.value.startsWith(".")
        ? col.value.substring(1)
        : col.value;
    if (tempColObj.setProperty) {
      tempColObj.setProperty =
        col.setProperty && col.setProperty.startsWith(".")
          ? col.setProperty.substring(1)
          : col.setProperty;
    }
    return tempColObj;
  });
...
const getDisplayFieldsMetaData = (columns) => {
  ...
  const itemsRecordsColumn = columns?.filter(
    (col) => col.itemsRecordsColumn === "true",
  );
  if (itemsRecordsColumn?.length > 0) {
    metaDataObj.itemsRecordsColumn = itemsRecordsColumn[0].value;
  }
  const itemsGroupKeyColumn = columns?.filter(
    (col) => col.itemsGroupKeyColumn === "true",
  );
...
const createSingleTreeObejct = (
  ...
  const isSelected = selected.some(
    (item) => item.id === entry[displayFieldMeta.key],
  );
...
const doSearch = async (
  ...
  if (dataApiObj) {
    ...
    if (isGroupData) {
      ...
      dataApiObj.parameters[Object.keys(dataApiObj.parameters)[0]] =
        initialCaseClass;
      ...
      if (clickedGroup) {
        ...
        if (searchText === "") {
          const containsData = itemsTree.find(
            (item) => item.id === clickedGroup,
          );
    ...
    // search API call
    const response = await dataApiObj
      .fetchData(searchTextForUngroupedData)
      .catch(() => {
        return itemsTree;
      });
    ...
    if (isGroupData) {
      if (searchText) {
        ...
        newItemsTree = putItemsDataInItemsTree(
          listObjData,
          displayFieldMeta,
          itemsTree,
          showSecondaryInSearchOnly,
    ...
    if (listObjData !== undefined && listObjData.length > 0) {
      newItemsTree = listObjData.map((entry) =>
        createSingleTreeObejct(
          entry,
          displayFieldMeta,
          showSecondaryData,
...
const setValuesToPropertyList = (
  searchText,
  assocProp,
  items,
  columns,
  actions,
  updatePropertyInRedux = true,
) => {
...
const getGroupDataForItemsTree = (
  groupDataSource,
