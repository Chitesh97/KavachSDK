...
export default function Multiselect(props) {
  ...
  const groupsDisplayFieldMeta = useMemo(
    () =>
      listType !== "associated"
        ? getDisplayFieldsMetaData(dataConfig.groupColumnsConfig)
        : null,
    [dataConfig.groupColumnsConfig],
  );

  const itemsTreeBaseData =
    getGroupDataForItemsTree(
      groupDataSource,
  ...
  const getCaseListBasedOnParams = async (
    searchText,
    group,
    selectedRows,
    currentItemsTree,
          ...
          return {
            id: item[
              selectionKey.startsWith(".")
                ? selectionKey.substring(1)
                : selectionKey
            ],
            primary:
              item[
                primaryField.startsWith(".")
    ...
    const initalItemsTree =
      isTriggeredFromSearch || !currentItemsTree
        ? [...itemsTreeBaseData]
        : [...currentItemsTree];
  ...
  useEffect(() => {
    getCaseListBasedOnParamsDebounced.current = debounce(
      500,
      getCaseListBasedOnParams,
    );
  }, []);
    ...
    if (listType !== "associated") {
      PCore.getDataApi()
        ...
        .then((dataObj) => {
          ...
          if (!isGroupData) {
            getCaseListBasedOnParamsDebounced.current(
              inputValue ?? "",
              "",
              [...selectedItems],
              [...itemsTree],
  ...
  const handleChange = (event, newSelectedValues) => {
    ...
    if (newSelectedValues.length > selectedItems.length) {
      clickedItem = newSelectedValues.filter(
        (item) => !selectedItems.some((ele: any) => ele.id === item.id),
      );
    } else {
      clickedItem = selectedItems.filter(
        (item: any) =>
          !newSelectedValues.some((ele: any) => ele.id === item.id),
      );
