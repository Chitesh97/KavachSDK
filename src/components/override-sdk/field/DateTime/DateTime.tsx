...
import {
  ...
  getDateFormatInfo,
} from "@pega/react-sdk-components/lib/components/helpers/date-format-utils";
...
export default function DateTime(props: DateTimeProps) {
  ...
  if (displayMode === "DISPLAY_ONLY") {
    ...
    return (
      <FieldValueList name={hideLabel ? "" : label} value={formattedDateTime} />
    );
  }
  ...
  const handleChange = (date) => {
    const timeZoneDateTime = (dayjs as any).tz(
      date.format("YYYY-MM-DDTHH:mm:ss"),
      timezone,
    );
    const changeValue =
      timeZoneDateTime && timeZoneDateTime.isValid()
        ? timeZoneDateTime.toISOString()
        : "";
  ...
  return (
    <DateTimePicker
      ...
      // autoOk
      disabled={disabled}
      inputFormat={`${dateFormatInfo.dateFormatString} hh:mm a`}
