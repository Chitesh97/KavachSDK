...
export const createFilterComponent = (getPConnect, filterMeta, index) => {
  ...
  let propInfo: any = PCore.getMetadataUtils().getPropertyMetadata(
    cleanedName,
  ...
  const isNumber =
    propertyType && (propertyType === "Decimal" || propertyType === "Integer");
  ...
  if (
    filterMeta.config.listType === "associated" &&
    propInfo &&
    propInfo.datasource
  ) {
        ...
        <Link
          ...
          onClick={() => {
            PCore.getPubSubUtils().publish(
              PCore.getConstants().PUB_SUB_EVENTS
                .EVENT_DASHBOARD_FILTER_CLEAR_ALL,
            );
          }}
          underline="hover"
        >
...
export const getFilterExpression = (filterValue, name, metadata) => {
  ...
  if (
    metadata.config.filterType &&
    metadata.config.filterType === "RelativeDates"
  ) {
    const fieldSource = metadata.config.datasource.filter(
      (source) => source.key === filterValue,
    )[0];
