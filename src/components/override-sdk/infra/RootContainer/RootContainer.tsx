...
function getItemView(routingInfo, renderingMode) {
  ...
  if (routingInfo && renderingModes.includes(renderingMode)) {
    const { accessedOrder, items }: { accessedOrder: any; items: any } =
      routingInfo;
...
export default function RootContainer(
  ...
  banners = hasBanner && (
    <div>
      {localizedVal(
        `RootContainer: trying to emit Banner with ${messages}`,
        localeCategory,
      )}
    </div>
  );
    ...
    switch (componentName) {
      case "View":
        noPortalContent = (
          <div>
            {localizedVal(
              "getNoPortalContent: RootContainer wants to render View in noPortal mode",
              localeCategory,
            )}
          </div>
        );
      ...
      case "ViewContainer": {
        ...
        const theViewContainerAsReact = createElement(
          createPConnectComponent(),
          theViewCont,
        );
      ...
      default:
        noPortalContent = (
          <div>
            {localizedVal(
              "getNoPortalContent: RootContainer wants to render NO componentName in noPortal mode",
              localeCategory,
            )}
          </div>
        );
  ...
  useEffect(() => {
    ...
    (PCore.getContainerUtils().getContainerAPI() as any).addContainerItems(
      items,
    );
  }, [routingInfo]);
  ...
  if (items.length > 0) {
    ...
    if (
      prevRootConfig === null ||
      !PCore.isDeepEqual(currentRootConfig, prevRootConfig)
    ) {
      rootView.current = createElement(
        createPConnectComponent(),
  ...
  if (renderingMode === "noPortal") {
    ...
    console.log(
      `${localizedVal("RootContainer rendering in noPortal mode", localeCategory)}`,
    );
  ...
  if (children && Children.count(children) > 0) {
    return (
      <>
        <div>
          {localizedVal(
            "RootContainer: Has children. Trying to show ModalManager with children, etc.",
            localeCategory,
          )}
        </div>
        {children}
  ...
  return (
    <div id="root-container">
      <div>
        {localizedVal(
          "RootContainer: Should be ModalManager, etc.",
          localeCategory,
        )}
      </div>
    </div>
