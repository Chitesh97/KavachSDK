...
import {
  useFocusFirstField,
  useScrolltoTop,
} from "@pega/react-sdk-components/lib/hooks";
...
export default function Assignment(props: PropsWithChildren<AssignmentProps>) {
  ...
  const cancelCreateStageAssignment =
    actionsAPI.cancelCreateStageAssignment.bind(actionsAPI);
  ...
  function getStepsInfo(steps, formedSteps: any = []) {
    steps.forEach((step) => {
      if (step.name) {
        step.name = PCore.getLocaleUtils().getLocaleValue(
          step.name,
          undefined,
          localeReference,
        );
  ...
  const scrollId = window.location.href.includes("embedded")
    ? "#pega-part-of-page"
    : "#portal";
        ...
        if (oCaseInfo?.navigation /* was oCaseInfo.navigation != null */) {
          ...
          if (oCaseInfo?.navigation?.steps) {
            const steps = JSON.parse(
              JSON.stringify(oCaseInfo?.navigation?.steps),
            );
      ...
      switch (sAction) {
        case "navigateToStep": {
          ...
          navigatePromise
            ...
            .catch(() => {
              showToast(
                `${localizedVal("Navigation failed!", localeCategory)}`,
              );
            });
        ...
        case "cancelAssignment": {
          ...
          const isAssignmentInCreateStage = thePConn
            .getCaseInfo()
  ...
  function getRefreshProps(refreshConditions) {
    ...
    return (
      refreshConditions
        .filter((item) => item.event && item.event === "Changes")
        .map((item) => [item.field, item.field?.substring(1)]) || []
    );
  }

  // expected format of refreshConditions : [{field: ".Name", event: "Changes"}]
  const refreshConditions = thePConn
    .getCaseInfo()
  ...
  if (refreshProps.length > 0) {
    refreshProps.forEach((prop) => {
      PCore.getRefreshManager().registerForRefresh(
        ...
        thePConn
          .getActionsApi()
          .refreshCaseView.bind(
            thePConn.getActionsApi(),
            caseKey,
            "",
            pageReference,
            {
