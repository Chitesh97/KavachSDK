...
export default function DeferLoad(props: DeferLoadProps) {
  ...
  const theRequestedAssignment = pConnect.getValue(
    PCore.getConstants().CASE_INFO.ASSIGNMENT_LABEL,
    "",
  ); // 2nd arg empty string until typedef allows optional
  ...
  const loadViewCaseID =
    ...
    pConnect.getValue(constants.CASE_INFO.CASE_INFO_ID, ""); // 2nd arg empty string until typedef allows optional
  let containerName;
  let containerItemData;
  const targetName = pConnect.getTarget();
  if (targetName) {
    containerName =
      PCore.getContainerUtils().getActiveContainerItemName(targetName);

    containerItemData = PCore.getContainerUtils().getContainerItemData(
      targetName,
      containerName,
    );
  }
  ...
  const getViewOptions = () => ({
    ...
    pageClass: loadViewCaseID
      ? ""
      : (pConnect.getDataObject("") as any).pyPortal.classID, // 2nd arg empty string until typedef allows optional
  ...
  const onResponse = (data) => {
    ...
    if (data && !(data.type && data.type === "error")) {
      ...
      configObject
        .getPConnect()
        .setInheritedProp("displayMode", "DISPLAY_ONLY");
      setContent(createElement(createPConnectComponent(), configObject));
      if (deferLoadId) {
        PCore.getDeferLoadManager().stop(
          deferLoadId,
          getPConnect().getContextName(),
        );
      }
  ...
  useEffect(() => {
    if (resourceType === DATA) {
      ...
      if (containerName) {
        const dataContext = PCore.getStoreValue(
          ".dataContext",
          "dataInfo",
          containerName,
        );
        ...
        console.error(
          "Cannot load the defer loaded view without container information",
        );
      }
