...
export default function View(props: PropsWithChildren<ViewProps>) {
  ...
  const { PAGE_TYPES: { PAGE, LANDINGPAGE, LISTPAGE } = {}, MODAL } =
    PCore.getConstants();
  ...
  useEffect(() => {
    ...
    applicationLabel = localeUtils.getLocaleValue(
      `${applicationLabel}`,
      "",
      "",
    );
    const caseInfo = getPConnect().getCaseInfo();
    const isAssignmentInCreateStage =
      caseInfo && caseInfo.isAssignmentInCreateStage();
    const isRenderingInModal = getPConnect().getContainerName().includes(MODAL);
    const isRenderingInPreviewPanel = getPConnect()
      .getContainerName()
      .includes("preview");
