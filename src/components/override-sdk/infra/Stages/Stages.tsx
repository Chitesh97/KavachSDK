...
export default function Stages(props: StagesProps) {
  ...
  const key =
    `${pConn.getCaseInfo().getClassName()}!CASE!${pConn.getCaseInfo().getName()}`.toUpperCase();

  const filteredStages = getFilteredStages(stages);
  const currentStageID = pConn.getValue(
    PCore.getConstants().CASE_INFO.STAGEID,
    "",
  ...
  function getStages(inStages) {
    return (
      <Breadcrumbs
        aria-label="stages"
        separator={<DoubleArrowIcon className={classes.separatorIcon} />}
      >
