...
export default function MultiStep(props: PropsWithChildren<MultiStepProps>) {
  ...
  let currentStep = arNavigationSteps.find(
    ({ visited_status: vs }) => vs === "current",
  );
  if (!currentStep) {
    const lastActiveStepIndex = arNavigationSteps.findLastIndex(
      ({ visited_status: vs }) => vs === "success",
    );
    currentStep =
      arNavigationSteps[lastActiveStepIndex >= 0 ? lastActiveStepIndex : 0];
  }
                          ...
                          <li className="psdk-sub-step-list">
                            <div style={{ display: "inline-flex" }}>
                              {subStep.visited_status === "current" && (
                                <img
                                  className="psdk-current-svg-icon"
                                  src="{svgCurrent}"
                                />
                              )}
                              {subStep.visited_status !== "current" && (
                                <img
                                  className="psdk-not-current-svg-icon"
                                  src="{svgNotCurrent}"
                                />
                              )}
                              {subStep.visited_status === "current" && (
                                <label className="psdk-sub-step-current">
                                  {subStep.name}
                                </label>
                              )}
                              {subStep.visited_status !== "current" && (
                                <label className="psdk-sub-step-not-current">
                                  {subStep.name}
                                </label>
                              )}
                            </div>
                ...
                <React.Fragment key={mainStep.actionID}>
                  <div className="psdk-horizontal-step-header">
                    ...
                    <div className={_getHLabelClass(mainStep)}>
                      <div
                        className="psdk-horizontal-step-text-label"
                        id="selected-label"
                      >
                        {mainStep.name}
                      </div>
          ...
          {arNavigationSteps.map((mainStep) => {
            return (
              <React.Fragment key={mainStep.actionID}>
                {mainStep.steps && (
                  <ul
                    style={{ paddingInlineStart: "0rem", marginLeft: "35px" }}
                  >
