...
const processRootViewDetails = (rootView, containerItem, options) => {
  ...
  const isAnnotedViewContext =
    PCore.getAnnotationUtils().isProperty(viewContext);

  // resolving annoted view context
  if (isAnnotedViewContext) {
    const viewContextProperty =
      PCore.getAnnotationUtils().getPropertyName(viewContext);
    resolvedViewContext = PCore.getStoreValue(
      `.${viewContextProperty}`,
      viewContextProperty.startsWith(".")
        ? parentGetPConnect().getPageReference()
        : "",
      containerContext,
    );
  }
  ...
  if (isAnnotedViewName) {
    const viewNameProperty =
      PCore.getAnnotationUtils().getPropertyName(viewName);
    resolvedViewName = PCore.getStoreValue(
      `.${viewNameProperty}`,
      resolvedViewContext,
      containerContext,
    );
...
export const getPConnectOfActiveContainerItem = (containerInfo, options) => {
  ...
  if (accessedOrder && items) {
    ...
    if (
      ...
      const target = activeContainerItemKey.substring(
        0,
        activeContainerItemKey.lastIndexOf("_"),
      );

      const { view: rootView, context } = activeContainerItem;
      const { viewName, viewContext } = processRootViewDetails(
        rootView,
        activeContainerItem,
        { parentGetPConnect },
      );
