...
export const FlowContainer = (props: FlowContainerProps) => {
  ...
  const getPConnect =
    getPConnectOfActiveContainerItem || getPConnectOfFlowContainer;
  const thePConn = getPConnect();
  const containerName =
    assignmentNames && assignmentNames.length > 0 ? assignmentNames[0] : "";
  ...
  const key =
    `${thePConn.getCaseInfo().getClassName()}!CASE!${thePConn.getCaseInfo().getName()}`.toUpperCase();
  ...
  useEffect(() => {
    ...
    if (
      isInitialized &&
      pConnectOfFlowContainer.getMetadata().children &&
      !hasItems
    ) {
  ...
  function isCaseWideLocalAction() {
    ...
    const actionID = ourPConn.getValue(
      pCoreConstants.CASE_INFO.ACTIVE_ACTION_ID,
      "",
    ); // 2nd arg empty string until typedefs properly allow optional
    const caseActions = ourPConn.getValue(
      pCoreConstants.CASE_INFO.AVAILABLEACTIONS,
      "",
    ); // 2nd arg empty string until typedefs properly allow optional
  ...
  function hasChildCaseAssignments() {
    ...
    const childCases = ourPConn.getValue(
      pCoreConstants.CASE_INFO.CHILD_ASSIGNMENTS,
      "",
    ); // 2nd arg empty string until typedefs properly allow optional
  ...
  useEffect(() => {
    ...
    const theCaseMessages = localizedVal(
      thePConn.getValue("caseMessages", ""),
      localeCategory,
    ); // 2nd arg empty string until typedefs properly allow optional
  ...
  const urgency = getPConnect().getCaseSummary().assignments
    ...
    : "";
  const operatorInitials = Utils.getInitials(
  ...
  return (
    <div
      ...
      id={buildName}
      className="psdk-flow-container-top"
    >
      {!bShowConfirm &&
        (!todo_showTodo ? (
          !displayOnlyFA ? (
            <Card className={`${classes.root} psdk-root`}>
              <CardHeader
                ...
                avatar={
                  <Avatar className={`${classes.avatar} psdk-avatar`}>
                    {operatorInitials}
                  </Avatar>
                }
              />
