...
function isOpenModalAction(prevModalCollection, currentModalList) {
  return prevModalCollection && currentModalList
    ? Object.keys(prevModalCollection).length < currentModalList.length
    : false;
}
...
function getKeyAndLatestItem(routinginfo, pConn, options) {
  ...
  if (
    PCore.getContainerUtils().hasContainerItems(buildName(pConn, containerName))
  ) {
    ...
    for (let i = accessedOrder.length - 1; i >= 0; i--) {
      ...
      if (
        (acTertiary && items[tempkey].acTertiary) ||
        (!acTertiary && !items[tempkey].acTertiary)
      ) {
        key = tempkey;
        break;
...
function getConfigObject(item, pConnect, isReverseCoexistence = false) {
  ...
  if (isReverseCoexistence) {
    config = {
      options: {
        ...
        hasForm: true,
        containerName:
          pConnect?.getContainerName() || PCore.getConstants().MODAL,
      },
    };
  ...
  if (item) {
    ...
    const target =
      PCore.getContainerUtils().getTargetFromContainerItemID(context);
    config = {
      meta: view,
      options: {
        context,
...
export default function ModalViewContainer(props: ModalViewContainerProps) {
  ...
  function showAlert(payload) {
    ...
    const isReverseCoexistence = (
      PCore.getCoexistenceManager().getBroadcastUtils() as any
    ).isReverseCoexistenceCaseLoaded();
    const { isModalAction, hideDelete, isDataObject, skipReleaseLockRequest } =
      payload;
  ...
  function handleModalClose(accessedOrder) {
    ...
    const [closedModalKey] = difference(
      Object.keys(tempModalCollection),
      accessedOrder,
    );
  ...
  function determineModalHeaderByAction(
    actionName,
    caseTypeName,
    ID,
    caseLocaleRef,
  ...
  useEffect(() => {
    if (routingInfo) {
      ...
      const { key, latestItem } = getKeyAndLatestItem(routingInfo, pConn, {
        acTertiary,
      });
      ...
      if (
        latestItem &&
        type === MULTIPLE &&
        (isOpenModalAction(modalCollection.current, accessedOrder) ||
          isUpdateModalAction(modalCollection.current, accessedOrder))
      ) {
        ...
        const isDataObject =
          routingInfo.items[latestItem.context].resourceType ===
          PCore.getConstants().RESOURCE_TYPES.DATA;
        const dataObjectAction =
          routingInfo.items[latestItem.context].resourceStatus;
        const isMultiRecord =
          routingInfo.items[latestItem.context].isMultiRecordData;
        const headingValue =
        ...
        if (pConnect.getComponentName() === "reference") {
          ...
          arChildrenAsReact = (pConnect.getChildren() as []).map(
            (child: any) => {
    ...
    <>
      <Dialog open={bShowModal} aria-labelledby="form-dialog-title">
        <DialogTitle
          ...
          className={`${classes.dlgTitle} psdk-dialog-title`}
        >
          {title}
        </DialogTitle>
      ...
      {bShowCancelAlert && (
        <CancelAlert {...cancelAlertProps} dismiss={dismissCancelAlert} />
      )}
    </>
  );
}
