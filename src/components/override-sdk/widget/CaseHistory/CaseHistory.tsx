...
export default function CaseHistory(props: CaseHistoryProps) {
  ...
  function computeRowData(rows: Object[]): void {
    ...
    rows.forEach((row: any, rowIndex: number) => {
      ...
      displayedColumns.forEach((column: any, rowValIndex) => {
        ...
        const theValue =
          theType === "Date" || theType === "DateTime"
            ? Utils.generateDateTime(row[theFieldName], "DateTime-Short")
            : row[theFieldName];
  ...
  function getTableHeader() {
    ...
    const theHeaderCells: any[] = displayedColumns.map((headerCol, index) => {
      ...
      return (
        <StyledTableCell key={theCellKey}>{headerCol.label}</StyledTableCell>
      );
    });
    ...
    if (rowData.current.length > 0) {
      rowData.current.forEach((dataRow: Object[], index) => {
        ...
        theDataRows.push(
          <TableRow key={theKey}>
            <StyledTableCell>
              {dataRow[0] ? dataRow[0] : ("---" as any)}
            </StyledTableCell>
            <StyledTableCell>
              {dataRow[1] ? dataRow[1] : ("---" as any)}
            </StyledTableCell>
            <StyledTableCell>
              {dataRow[2] ? dataRow[2] : ("---" as any)}
            </StyledTableCell>
          </TableRow>,
        );
      });
