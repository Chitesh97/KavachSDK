...
export default function FileUtility(props: FileUtilityProps) {
  ...
  const ActionButtonsForFileUtil = getComponentFromMap(
  ...
  const headerSvgIcon$ = Utils.getImageSrc(
    "paper-clip",
  ...
  const configProps: any = thePConn.resolveConfigProps(
    thePConn.getConfigProps(),
  );
      ...
      const actionsMap = new Map([
        [
          "download",
          {
            id: `download-${ID}`,
            text: isFile
              ? thePConn.getLocalizedValue("Download", "", "")
              : thePConn.getLocalizedValue("Open", "", ""), // 2nd and 3rd args empty string until typedef marked correctly
      ...
      attPromise.then((resp: any) => {
        ...
        const arItems: any = arFullListAttachments.slice(0, 3).map((att) => {
          return getListUtilityItemProps({
            att,
            downloadFile: !att.progress
              ? () => downloadAttachedFile(att)
              : null,
            cancelFile: null,
  ...
  useEffect(() => {
    PCore.getPubSubUtils().subscribe(
      (PCore.getEvents().getCaseEvent() as any)
        .CASE_ATTACHMENTS_UPDATED_FROM_CASEVIEW,
      getAttachments,
  ...
  function onAttachFiles() {
    ...
    Promise.allSettled(
      fileData.attachedFiles.map((file) =>
        attachmentUtils.uploadAttachment(
          file,
          onUploadProgress,
          errorHandler,
          thePConn.getContextName(),
        ...
        <div className="psdk-dialog-background">
          <div className="psdk-modal-file-top">
            <div className="psdk-view-all-header">
              ...
              <button
                type="button"
                className="psdk-close-button"
                onClick={() => setViewAll(false)}
              >
                <img
                  className="psdk-utility-card-actions-svg-icon"
                  src={closeSvgIcon}
                />
              </button>
            </div>
