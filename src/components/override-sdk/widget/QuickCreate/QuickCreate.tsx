    ...
    if (envInfo?.environmentInfoObject?.pyCaseTypeList) {
      envInfo.environmentInfoObject.pyCaseTypeList.forEach((casetype) => {
        if (
          ...
          casetype.pyWorkTypeImplementationClassName
        ) {
          defaultCases.push({
      ...
      const pyPortalInAppContext = pConnectInAppContext.getValue(
        "pyPortal",
      ) as any;
      pyPortalInAppContext?.pyCaseTypesAvailableToCreate?.forEach(
        (casetype) => {
