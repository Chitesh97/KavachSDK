...
const getAttachmentKey = (name = "") =>
  name ? `attachmentsList.${name}` : "attachmentsList";
...
export default function Attachment(props: AttachmentProps) {
  ...
  const caseID = PCore.getStoreValue(
    ".pyID",
    "caseInfo.content",
  ...
  const uploadMultipleFilesLabel = localizedVal(
    "file_upload_text_multiple",
    localeCategory,
  );
  const uploadSingleFileLabel = localizedVal(
    "file_upload_text_one",
    localeCategory,
  );
  let categoryName = "";
  ...
  const srcImg = Utils.getImageSrc(
    "document-doc",
  ...
  const [files, setFiles] = useState<any[]>(() =>
    value?.pxResults && +value.pyCount > 0
      ? value.pxResults.map((f) => buildFilePropsFromResponse(f))
      : [],
  );
  ...
  const deleteFile = useCallback(
    (file) => {
      ...
      if (
        ...
        +value?.pyCount > 0 &&
        file.responseProps &&
        file?.responseProps?.pzInsKey !== "temp"
      ) {
        const updatedAttachments = files.map((f) => {
          if (
            f.responseProps &&
            f.responseProps.pzInsKey === file.responseProps.pzInsKey
          ) {
        ...
        updateAttachmentState(pConn, getAttachmentKey(valueRef), [
          ...updatedAttachments,
        ]);
        setFiles((current) => {
        ...
        if (file.inProgress) {
          // @ts-ignore - 3rd parameter "responseEncoding" should be optional
          PCore.getAttachmentUtils().cancelRequest(
            file.ID,
            pConn.getContextName(),
          );
        }
  ...
  const errorHandler = (isFetchCanceled, attachedFile) => {
    return (error) => {
      if (!isFetchCanceled(error)) {
        let uploadFailMsg = pConn.getLocalizedValue(
        ...
        if (
          ...
          uploadFailMsg = pConn.getLocalizedValue(
        ...
        setFiles((current) => {
          return current.map((f) => {
            if (f.ID === attachedFile.ID) {
              ...
              f.props.name = pConn.getLocalizedValue(
              ...
              PCore.getMessageManager().addMessages({
                messages: [
                  {
                    type: "error",
                    message: pConn.getLocalizedValue(
                      "Error with one or more files",
                      "",
                      "",
                    ),
                  },
  ...
  const onFileAdded = (event) => {
    ...
    const maxAttachmentSize =
      PCore.getEnvironmentInfo().getMaxAttachmentSize() || "5";
      ...
      ...addedFiles.map((f: any, index) => {
        ...
        if (!validateMaxSize(f, maxAttachmentSize)) {
          ...
          f.props.meta = pConn.getLocalizedValue(
            `File is too big. Max allowed size is ${maxAttachmentSize}MB.`,
            "",
            "",
          );
    ...
    const tempFilesWithError = tempFilesToBeUploaded.filter(
      (f) => f.props.error,
    );
    ...
    setFiles((current) =>
      allowMultiple !== "true"
        ? [...tempFilesToBeUploaded]
        : [...current, ...tempFilesToBeUploaded],
    );
  ...
  const uploadFiles = useCallback(() => {
    const filesToBeUploaded = files
      .filter((e) => {
        ...
        const isFileUploadedinLastStep =
          e.responseProps && e.responseProps.pzInsKey;
        ...
        if (fileResponses.length > 0) {
          setFiles((current) => {
            ...
            tempFilesUploaded.forEach((f) => {
              const index = fileResponses.findIndex(
                (fr: any) => fr.value.clientFileID === f.ID,
              );
              if (index >= 0) {
                f.props.meta = pConn.getLocalizedValue(
  ...
  useEffect(() => {
    if (files.length > 0 && displayMode !== "DISPLAY_ONLY") {
      ...
      const tempFiles = files.filter(
        (f) =>
          currentAttachmentList.findIndex((fr) => fr.ID === f.ID) === -1 &&
          !f.inProgress &&
          f.responseProps,
      );
  ...
  useEffect(() => {
    ...
    setFiles((current) => {
      return [
        ...current.map((f) => {
          return f.responseProps.pzInsKey &&
            !f.responseProps.pzInsKey.includes("temp")
            ? {
                ...f,
                props: {
                  ...f.props,
  ...
  const content = (
    <div style={{ marginBottom: "8px" }}>
      <div
        className={`${disabled ? "file-disabled" : ""} ${validatemessage === "" ? "file-div" : "file-div-error"}`}
      >
          ...
          return (
            <div key={index} className="psdk-utility-card">
              ...
              <div className="psdk-utility-card-main">
                <div className="psdk-utility-card-main-primary-label">
                  {item.props.name}
                </div>
                {item.props.meta && (
                  <div style={{ color: item.props.error ? "red" : undefined }}>
                    {item.props.meta}
                  </div>
                )}
              </div>
                  ...
                  <div>
                    ...
                    >
                      <MenuItem
                        ...
                        onClick={() =>
                          downloadFile(
                            item.responseProps ? item.responseProps : {},
                          )
                        }
                      >
                        Download
  ...
  return (
    <div className="file-upload-container">
      ...
      {((files.length === 0 && allowMultiple !== "true") ||
        allowMultiple === "true") && <section>{content}</section>}
      {validatemessage !== "" ? (
        <span className="file-error">{validatemessage}</span>
      ) : (
        <span style={{ fontSize: "14px" }}>{helperText}</span>
      )}
