...
export default function CaseSummaryFields(props: CaseSummaryFieldsProps) {
  ...
  function getFieldValue(field: any): any {
    ...
    switch (fieldTypeLower) {
      ...
      case "phone": {
        const displayPhone =
          field.config.value !== "" ? field.config.value : "---";
        return (
      ...
      case "currency": {
        const theCurrencyOptions = getCurrencyOptions(
          field.config?.currencyISOCode,
        );
        return (
          <TextField
            variant="standard"
  ...
  useEffect(() => {
    const arGridItems = theFieldsToRender.map((field: any) => {
      ...
      if (
        field.config.visibility === undefined ||
