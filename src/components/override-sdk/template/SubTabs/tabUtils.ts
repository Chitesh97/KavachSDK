  ...
  return allTabs.props
    ...
    ?.filter((child) => {
      ...
      if (child.getPConnect().getComponentName() === "DeferLoad") {
        const { name: viewName, deferLoadId = `${viewName}_${uuid}_${index}` } =
          config;
...
export const tabClick = (
  ...
  PCore.getDeferLoadManager().deactivate(
    deferLoadId,
    currentPConn.getContextName(),
  );
  ...
  PCore.getDeferLoadManager().refreshComponent(
    activeId,
    nextPConn?.getContextName(),
  );
};
