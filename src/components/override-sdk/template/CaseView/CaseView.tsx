...
export default function CaseView(props: PropsWithChildren<CaseViewProps>) {
  ...
  function getChildRegionByName(inName: string): any {
    for (const child of children as ReactElement[]) {
      const theMetadataType: string = (child as ReactElement).props
        .getPConnect()
        .getRawMetadata()
        .type.toLowerCase();
      const theMetadataName: string = (child as ReactElement).props
        .getPConnect()
        .getRawMetadata()
        .name.toLowerCase();
  ...
  if (theTabsRegionChildren) {
    ...
    theTabsRegionChildren.forEach((tabComp, index) => {
      ...
      if (
        theTabCompConfig.visibility === undefined ||
        theTabCompConfig.visibility === true
      ) {
