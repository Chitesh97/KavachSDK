...
export default function AppShell(props: PropsWithChildren<AppShellProps>) {
  ...
  useEffect(() => {
    ...
    const tempMap: any = (pConn.getChildren() as any)?.map(
      (child: any, index) => {
        ...
        return (
          <div
            id={theChildComp}
            key={theKey}
            style={{ border: "solid 1px silver", margin: "1px" }}
          >
  ...
  useEffect(() => {
    // @ts-ignore
    const caseTypesAvailableToCreateDP =
      PCore.getEnvironmentInfo().environmentInfoObject?.pxApplication
        ?.pyCaseTypesAvailableToCreateDP;
    if (caseTypesAvailableToCreateDP) {
      const portalID = pConn.getValue(".pyOwner");
      PCore.getDataPageUtils()
        .getPageDataAsync(
          caseTypesAvailableToCreateDP,
        ...
        .then((response: { pyCaseTypesAvailableToCreate?: any }) => {
          if (response?.pyCaseTypesAvailableToCreate) {
            pConn.replaceState(
              ".pyCaseTypesAvailableToCreate",
              response.pyCaseTypesAvailableToCreate,
              {
      ...
      <div id="AppShell">
        <WssNavBar
          ...
          appInfo={{
            ...
            onClick:
              links[0] &&
              /* links[0].onClick ? */ links[0].onClick /* : undefined */,
          }}
