...
export default function DataReference(
  props: PropsWithChildren<DataReferenceProps>,
) {
  // Get emitted components from map (so we can get any override that may exist)
  const SingleReferenceReadonly = getComponentFromMap(
  ...
  let propName;
  const isDisplayModeEnabled = ["STACKED_LARGE_VAL", "DISPLAY_ONLY"].includes(
    displayMode,
  );
  ...
  if (firstChildMeta?.type !== "Region") {
    ...
    if (firstChildMeta?.type === "Dropdown") {
      firstChildMeta.config.datasource.source = rawViewMetadata.config
        ?.parameters
        ? dropDownDataSource
    ...
    if (
      firstChildMeta.type === "SimpleTableSelect" &&
      selectionMode === SELECTION_MODE.MULTI
    ) {
      propName = PCore.getAnnotationUtils().getPropertyName(
        firstChildMeta.config.selectionList,
      );
    } else {
      propName = PCore.getAnnotationUtils().getPropertyName(
        firstChildMeta.config.value,
      );
    }
  }
