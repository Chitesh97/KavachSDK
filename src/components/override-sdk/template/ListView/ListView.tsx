...
export default function ListView(props: ListViewProps) {
  ...
  const rowID =
    compositeKeys && compositeKeys?.length === 1 ? compositeKeys[0] : defRowID;
  ...
  function getComparator<T, Key extends keyof T>(
    ...
    return theOrder === "desc"
      ? (a: T, b: T) => descendingComparator(a, b, orderedBy)
      : (a: T, b: T) => -descendingComparator(a, b, orderedBy);
  }
  ...
  function getHeaderCells(colFields, fields) {
    return colFields.map((field: any, index) => {
      let theField = field.config.value.substring(
        field.config.value.indexOf(" ") + 1,
      );
      ...
      if (displayAsLink) {
        headerRow.isAssignmentLink = AssignDashObjects.includes(
          headerRow.classID,
        );
  ...
  function processFilterChange(data) {
    ...
    if (
      data.filterExpression !== null &&
      !(columnList.current?.length && columnList.current?.includes(field))
    ) {
      return;
    }
  ...
  async function fetchDataFromServer() {
    ...
    const tableDataResults = !bInForm
      ? workListJSON.data.data
      : workListJSON.data;
  ...
  function openWork(row) {
    ...
    if (pxRefObjectClass !== "" && pxRefObjectKey !== "") {
      thePConn
        .getActionsApi()
        .openWorkByHandle(pxRefObjectKey, pxRefObjectClass);
    }
  }
  ...
  function _listTitle() {
    ...
    let title =
      resolvedConfigProps.title || resolvedConfigProps?.label || defaultTitle;
  ...
  const processColumnValue = (column, value) => {
    ...
    switch (type) {
      ...
      case "Currency":
        theCurrencyOptions = getCurrencyOptions(
          PCore?.getEnvironmentInfo()?.getLocale() as string,
        );
        ...
        <Paper className={classes.paper}>
          ...
          {globalSearch && (
            ...
            >
              ...
              <Grid item>
                <TextField
                  label={PCore.getLocaleUtils().getLocaleValue(
                    "Search",
                    "Search",
                  )}
                  fullWidth
