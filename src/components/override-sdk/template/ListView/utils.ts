...
export function updateMetaEmbeddedFieldID(metaFields) {
  return metaFields.forEach((metaField) => {
    if (
      metaField.fieldID?.startsWith(PAGE) ||
      metaField.fieldID?.startsWith(PAGELIST)
    ) {
...
export const isPageListInPath = (propertyName, currentClassID) => {
  ...
  const metadata: any = PCore.getMetadataUtils().getPropertyMetadata(
    first,
    currentClassID,
  );
...
export function getConfigEmbeddedFieldsMeta(configFields, classID) {
  ...
  configFields.forEach((field) => {
    ...
    if (isEmbeddedField(value)) {
      ...
      const meta: any = PCore.getMetadataUtils().getEmbeddedPropertyMetadata(
        value,
        classID,
      );
      meta.fieldID = field;
 ...
 * @param {Array} metaFields  Fields metadata Array. Contains metadata of all the fields
 */
export function mergeConfigEmbeddedFieldsMeta(
  configEmbeddedFieldsMeta,
  metaFields,
) {
  const mergedMetaFields = [...metaFields];
  configEmbeddedFieldsMeta.forEach((configFieldMeta) => {
    const fieldMeta = metaFields.find(
      (metaField) => metaField.fieldID === configFieldMeta.fieldID,
    );
...
function updateFieldType(metaFields) {
  metaFields.forEach((metaField) => {
    if (metaField.type)
      metaField.type =
        oldToNewFieldTypeMapping[metaField.type] || metaField.type;
  });
}
...
function getConfigFields(configFields, primaryFields, metaFields, classID) {
  ...
  if (primaryFieldsViewIndex > -1) {
    ...
    const uncommonFieldsList = primaryFields.filter(
      (primaryField) =>
        !presetConfigFields.some(
          (presetConfigField) =>
            presetConfigField.config.value.split(".")[1] === primaryField,
        ),
    );
    const uncommonFieldsRawMeta: any[] = [];
    uncommonFieldsList.forEach((uncommonField) => {
      const uncommonFieldMeta = metaFields.find(
        (metaField) => metaField.fieldID === uncommonField,
      );
      if (uncommonFieldMeta) uncommonFieldsRawMeta.push(uncommonFieldMeta);
    });
    const uncommonFieldsConfigMeta = generateViewMetaData(
      uncommonFieldsRawMeta,
      classID,
      true,
    );

    presetConfigFields.splice(
      primaryFieldsViewIndex,
      1,
      ...uncommonFieldsConfigMeta,
    );
  }
  return presetConfigFields;
}
...
export function getTableConfigFromPresetMeta(
  ...
  if (isMetaWithPresets) {
    ...
    cardHeader =
      presetMeta.cardHeader && getPresetMetaAttribute(presetMeta.cardHeader);
    secondaryText =
      presetMeta.secondaryText &&
      getPresetMetaAttribute(presetMeta.secondaryText);
    timelineDate =
      presetMeta.timelineDate &&
      getPresetMetaAttribute(presetMeta.timelineDate);
    timelineTitle =
      presetMeta.timelineTitle &&
      getPresetMetaAttribute(presetMeta.timelineTitle);
    timelineStatus =
      presetMeta.timelineStatus &&
      getPresetMetaAttribute(presetMeta.timelineStatus);
    timelineIcon =
      presetMeta.timelineIcon &&
      getPresetMetaAttribute(presetMeta.timelineIcon);
    ...
    if (
      ...
      !fieldsMeta.children.find((fieldMeta) => {
        return (
          fieldMeta?.config?.value === presetMeta.timelineTitle?.config?.value
        );
      })
    ) {
    ...
    if (
      ...
      !fieldsMeta.children.find((fieldMeta) => {
        return (
          fieldMeta?.config?.value === presetMeta.timelineDate?.config?.value
        );
      })
    ) {
...
function populateRenderingOptions(name, config, field) {
  const shouldDisplayAsSemanticLink =
    "displayAsLink" in field.config && field.config.displayAsLink;
  if (shouldDisplayAsSemanticLink) {
    config.customObject.isAssignmentLink = AssignDashObjects.includes(
      field.config.classID,
    );
...
export function preparePatchQueryFields(
  fields,
  isDataObject = false,
  classID = "",
) {
  ...
  fields.forEach((field) => {
    ...
    if (field.cellRenderer === "WorkLink") {
      if (field.customObject && field.customObject.isAssignmentLink) {
        const associationName = field.name.includes(":")
          ? `${field.name.split(":")[0]}:`
          : "";
        ...
        const dataViewName =
          PCore.getDataTypeUtils().getSavableDataPage(classID);
        const dataPageKeys =
          PCore.getDataTypeUtils().getDataPageKeys(dataViewName);
        dataPageKeys?.forEach((item) =>
          item.isAlternateKeyStorage
            ? patchFields.push(item.linkedField)
            : patchFields.push(item.keyName),
        );
      } else {
...
export function updatePageFieldsConfig(configFields, parentClassID) {
  return configFields.forEach((item) => {
    ...
    if (
      isEmbeddedField(value) &&
      !isPageListInPath(propertyName, parentClassID)
    ) {
...
export const readContextResponse = async (context, params) => {
  ...
  if (showDynamicFields) {
    const sourceMetadata: any =
      PCore.getMetadataUtils().getDataPageMetadata(referenceList);
  ...
  if (isDataObject) {
    ...
    dataPageKeys?.forEach((item) =>
      item.isAlternateKeyStorage
        ? compositeKeys.push(item.linkedField)
        : compositeKeys.push(item.keyName),
    );
  ...
  childrenIterator?.forEach((presetMeta, index) => {
    ...
    const reportColumnsSet = isQueryable
      ? getReportColumns(promisesResponseArray[1])
      : new Set();

    const configFieldSet = prepareConfigFields(
      configFields,
      pushToComponentsList,
    );
    ...
    const configEmbeddedFieldsMeta = getConfigEmbeddedFieldsMeta(
      configFieldSet,
      classID,
    );
    const mergedMetaFields = mergeConfigEmbeddedFieldsMeta(
      configEmbeddedFieldsMeta,
      metaFields,
    );
    ...
    const patchQueryFields = preparePatchQueryFields(
      fields,
      isDataObject,
      classID,
    );
