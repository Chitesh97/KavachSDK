...
export default function useInit(props) {
  ...
  useEffect(() => {
    ...
    (async function init() {
      ...
      const metaDataPromise: Promise<any> =
        PCore.getAnalyticsUtils().getDataViewMetadata(
          referenceList,
          showDynamicFields,
        );
      ...
      const reportColumnsPromise = (
        PCore.getAnalyticsUtils().getFieldsForDataSource(
          referenceList,
          false,
      ...
      const fetchEditDetails = async (metadata) => {
        ...
        if (!isDataObject) {
          ...
          const { MULTI_ON_HOVER, MULTI } =
            PCore.getConstants().LIST_SELECTION_MODE;
      ...
      const editPromise = metaDataPromise.then((metadata) =>
        fetchEditDetails(metadata),
      );
