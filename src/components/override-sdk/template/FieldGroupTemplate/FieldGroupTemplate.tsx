...
import {
  getReferenceList,
  buildView,
} from "@pega/react-sdk-components/lib/components/helpers/field-group-utils";
...
export default function FieldGroupTemplate(props: FieldGroupTemplateProps) {
  ...
  const isReadonlyMode =
    renderMode === "ReadOnly" || displayMode === "DISPLAY_ONLY";
  ...
  const addRecord = () => {
    if (PCore.getPCoreVersion()?.includes("8.7")) {
      pConn
        .getListActions()
        .insert({ classID: contextClass }, referenceList.length, pageReference);
    } else {
