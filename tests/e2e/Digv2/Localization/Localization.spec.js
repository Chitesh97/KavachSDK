...
test.describe("E2E test", () => {
  test("should login, create case and test the localized values", async ({
    page,
  }) => {
    await common.login(
    ...
    expect(
      await page.locator('div[role="button"]:has-text("Hogar")'),
    ).toBeVisible(); // Home
    expect(
      await page.locator(
        'div[role="button"]:has-text("Panel de control en línea")',
      ),
    ).toBeVisible(); // Inline Dashboard

    /** Creating a Complex Fields case-type */
    const complexFieldsCase = page.locator(
      'div[role="button"]:has-text("Campos complejos")',
    );
    await complexFieldsCase.click();

    /** Testing Case summary */
    expect(
      await page.locator('div[id="case-name"]:has-text("Campos complejos")'),
    ).toBeVisible(); // case type

    expect(
      await page.locator('button[id="edit"]:has-text("Editar")'),
    ).toBeVisible(); // edit action
    expect(
      await page.locator(
        'button[id="actions-menu"]:has-text("Comportamiento")',
      ),
    ...
    await expect(
      caseHistory.locator('th >> text="Interpretado por"'),
    ).toBeVisible();
    ...
    await expect(
      assignmentHeader.locator('h6:has-text("Seleccionar prueba")'),
    ).toBeVisible();
    await expect(
      assignmentHeader.locator('span:has-text("Tarea en")'),
    ).toBeVisible();

    /** Testing action buttons */
    const assignment = await page.locator('div[id="Assignment"]');
    await expect(
      assignment.locator('button:has-text("Cancelar")'),
    ).toBeVisible();
    await expect(
      assignment.locator('button:has-text("Entregar")'),
    ).toBeVisible();
    ...
    await expect(
      assignment.locator(
        'div[id="selected-label"]:has-text("Datos integrados")',
      ),
    ).toBeVisible();
    ...
    expect(
      assignment.locator('label >> text="Número de teléfono"'),
    ).toBeVisible();

    await page.locator('button:has-text("Next")').click();

    await expect(
      assignment.locator('h3:has-text("Dirección de Envío")'),
    ).toBeVisible();
    ...
    await expect(
      assignment.locator('th >> text="Código Postal"'),
    ).toBeVisible();
    await expect(
      assignment.locator('th >> text="Número de teléfono"'),
    ).toBeVisible();

    /** Testing file utility */
    const fileUtility = await page.locator('div[id="file-utility"]');
    await expect(
      fileUtility.locator('div:has-text("Archivos adjuntos0")'),
    ).toBeVisible();
    ...
    await expect(
      fileUtility.locator('h3:has-text("Agregar archivos locales")'),
    ).toBeVisible();
    await expect(
      fileUtility.locator('button:has-text("Cancelar")'),
    ).toBeVisible();
    await expect(
      fileUtility.locator('button:has-text("Adjuntar archivos")'),
    ).toBeVisible();
    ...
    await expect(
      fileUtility.locator('h3:has-text("Agregar enlaces")'),
    ).toBeVisible();
    await expect(
      fileUtility.locator('div >> text="Añadir enlace"'),
    ).toBeVisible();
    ...
    await expect(
      fileUtility.locator('button:has-text("Adjuntar enlaces")'),
    ).toBeVisible();
