...
test.describe("E2E test", () => {
  test("should login, create case and run different test cases for Dynamic Tabs", async ({
    ...
    const tablist = await page.locator(
      'div[id="dynamic-tabs"] div[role="tablist"]',
    );
    const tabpanel = await page.locator('div[id="dynamic-tabpanel"]');

    await expect(
      tabpanel.nth(0).locator('span:has-text("Make")'),
    ).toBeVisible();
    await expect(tabpanel.nth(0).locator('span:has-text("BMW")')).toBeVisible();

    await tablist.locator("button").nth(1).click();
    expect(
      await tabpanel.nth(1).locator('span:has-text("Make")'),
    ).toBeVisible();
    expect(
      await tabpanel.nth(1).locator('span:has-text("Audi")'),
    ).toBeVisible();

    await tablist.locator("button").nth(2).click();
    expect(
      await tabpanel.nth(2).locator('span:has-text("Make")'),
    ).toBeVisible();
    expect(
      await tabpanel.nth(2).locator('span:has-text("FIAT")'),
    ).toBeVisible();

    await tablist.locator("button").nth(3).click();
    expect(
      await tabpanel.nth(3).locator('span:has-text("Make")'),
    ).toBeVisible();
    expect(
      await tabpanel.nth(3).locator('span:has-text("Chevrolet")'),
    ).toBeVisible();
  }, 10000);
});
