...
test.describe("E2E test", () => {
  ...
  test("should login, create case and run the Attachment tests", async ({
    page,
  }) => {
    await common.login(
    ...
    const zeroBytesFile = path.join(
      __dirname,
      "../../../../assets/img/Zerobytes",
    );
    ...
    await expect(
      page.locator('span:has-text("Cannot be blank")'),
    ).toBeVisible();
    ...
    // Disable tests
    const alwaysDisabledAttachment = page.locator(
      'input[id="AttachmentDisabledAlways"]',
    );
    attributes = await common.getAttributes(alwaysDisabledAttachment);
    await expect(attributes.includes("disabled")).toBeTruthy();

    const conditionallyDisabledAttachment = page.locator(
      'input[id="AttachmentDisabledCondition"]',
    );
    ...
    const neverDisabledAttachment = page.locator(
      'input[id="AttachmentDisabledNever"]',
    );
    ...
    await expect(
      multipleAttachment.locator(
        'span[role="button"]:has-text("Choose files")',
      ),
    ).toBeVisible();
    ...
    await expect(
      page.locator('div >> text="Uploaded successfully" >> nth=0'),
    ).toBeVisible();
    ...
    await expect(
      page.locator('div >> text="Error with one or more files"'),
    ).toBeVisible();
    await expect(
    ...
    /** Download attachment */
    const menuSelector = await page.locator(
      'div[id="file-menu"]:not([aria-hidden="true"])',
    );
